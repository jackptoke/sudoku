...
module FileManipulator
  def self.split_and_save_datas(datas)
    count = 1
  ...
  def self.load_sudoku_from_file(filename)
    sudoku_game = []
    begin
      File.open(filename, 'r').each do |line|
        sudoku_game << line.strip.split(',').map(&:to_i)
        unless soduku_format?(sudoku_game[-1])
  ...
  def self.load_batch_sudokus_from_file(filename)
    sudoku_game = []
  ...
  def self.soduku_format?(row)
    return false if row.size != 9

    row.each do |value|
      return false if value.to_i.negative? || value.to_i > 9
    end
    true
  end
end
